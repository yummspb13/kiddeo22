...
export default function ProfileReviews({ onStatsUpdate }: ProfileReviewsProps) {
  ...
  const fetchUser = async () => {
    try {
      console.log('🔍 Fetching current user for reviews')
      const response = await fetch('/api/simple-user')
      if (response.ok) {
        const data = await response.json()
        console.log('✅ User data for reviews:', data)
        if (data.success) {
          setUser(data.user)
  ...
  const fetchReviews = async (userId?: number, page: number = 1) => {
    ...
    try {
      console.log('🔍 Fetching reviews for user:', currentUserId, 'page:', page)
    ...
    <div className="space-y-6">
      ...
      {reviews.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Статистика отзывов</h3>
