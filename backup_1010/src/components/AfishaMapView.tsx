...
export default function AfishaMapView({ events, cityCenter, cityName }: AfishaMapViewProps) {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      setMapError('Карта не загрузилась. Возможно, проблема с сетью.')
    }, 5000) // 5 секунд
    ...
    const loadYandexMaps = () => {
      ...
      if (typeof window !== 'undefined' && (window as any).__yandexMapsLoading) {
      ...
      // Проверяем ранее подключенные скрипты
      const existingScripts = Array.from(document.querySelectorAll('script[src*="api-maps.yandex.ru"]')) as HTMLScriptElement[]
      
      if (existingScripts.length) {
        // Если скрипт уже есть, просто ждем его загрузки
          if (!scriptListenerAttachedRef.current) {
            existingScripts[0].addEventListener('load', initMap)
            scriptListenerAttachedRef.current = true
          }
          return
        }

      // Помечаем что загружаем
      if (typeof window !== 'undefined') {
        (window as any).__yandexMapsLoading = true
      }
      ...
      script.onload = () => {
        if (typeof window !== 'undefined') {
          (window as any).__yandexMapsLoading = false
        }
        initMap()
      }
      script.onerror = () => {
        ...
        setMapError('Ошибка загрузки Яндекс.Карт')
      }
              ...
              <div style="display:flex;gap:12px;align-items:center;height:180px">
                <a href="/event/${markerData.event.slug || markerData.event.id}" style="display:block;width:140px;height:160px;border-radius:10px;overflow:hidden;background:#f3f4f6;flex-shrink:0;position:relative">
                  ...
                  <div style="position:absolute;bottom:0;left:0;background:rgba(0,0,0,0.7);color:white;padding:4px 8px;font-size:10px;font-weight:600;border-radius:0 6px 0 0;text-transform:uppercase;letter-spacing:0.5px">
                    ${markerData.event.category || 'Событие'}
                  </div>
                </a>
                <div style="min-width:0;flex:1;display:flex;flex-direction:column;height:160px">
                  ...
                  <div style="color:#374151;font-size:12px">
                    ${new Date(markerData.event.startDate).toLocaleDateString('ru-RU',{day:'2-digit',month:'2-digit',year:'numeric'})}
                    • ${new Date(markerData.event.startDate).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}
                    ${markerData.event.endDate ? ` — ${new Date(markerData.event.endDate).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}` : ''}
                  </div>
                  </div>
