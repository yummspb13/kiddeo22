...
export default function VenuePartnersClient() {
  ...
  const fetchSubcategoryData = async (subcategoryId: string) => {
    ...
    try {
      ...
      const [parametersRes, filtersRes] = await Promise.all([
        fetch(`/api/admin/venues/parameters?subcategoryId=${subcategoryId}&key=${key}`),
        fetch(`/api/admin/venues/filters?subcategoryId=${subcategoryId}&key=${key}`)
      ])
  ...
  const handleModerationAction = async (action: 'approve' | 'reject' | 'correction') => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/venues/partners/${moderationPartner.id}?key=${key}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingPartner 
        ? `/api/admin/venues/partners/${editingPartner.id}?key=${key}`
        : `/api/admin/venues/partners?key=${key}`
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/admin/venues/partners/${id}?key=${key}`, {
        method: 'DELETE',
      })
