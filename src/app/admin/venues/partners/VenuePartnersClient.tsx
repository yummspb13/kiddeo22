...
export default function VenuePartnersClient() {
  ...
  const [formData, setFormData] = useState({
    ...
    subcategoryId: '',
    vendorId: '',
    tariff: 'FREE' as 'FREE' | 'SUPER' | 'MAXIMUM',
    cityId: '',
    priceFrom: '',
    ageFrom: ''
  })
  ...
  useEffect(() => {
    console.log('🔍 useEffect called, key:', key)
    fetchData()
  }, [])
  ...
  const fetchData = async () => {
    try {
      ...
      const [partnersRes, subcategoriesRes, citiesRes] = await Promise.all([
        fetch(`/api/admin/venues/partners?key=${key}`),
        fetch(`/api/admin/venues/subcategories?key=${key}`),
        fetch('/api/cities')
      ])
      ...
      if (subcategoriesRes.ok) {
        ...
        console.log('Subcategories state updated')
      } else {
  ...
  const fetchSubcategoryData = async (subcategoryId: string) => {
    ...
    try {
      ...
      const [parametersRes, filtersRes] = await Promise.all([
        fetch(`/api/admin/venues/parameters?subcategoryId=${subcategoryId}${key}`),
        fetch(`/api/admin/venues/filters?subcategoryId=${subcategoryId}${key}`)
      ])
  ...
  const searchVendors = async (searchTerm: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/venues/vendors?search=${encodeURIComponent(searchTerm)}&key=${key}`)
      if (response.ok) {
        const data = await response.json()
        // API возвращает объект с полем vendors, извлекаем его
        const vendorsData = data.vendors || data
        // Преобразуем в простой формат для выпадающего списка
        const simpleVendors = vendorsData.map((vendor: any) => ({
          id: vendor.vendorId || vendor.id,
          displayName: vendor.vendor?.displayName || vendor.displayName || 'Неизвестный вендор'
        }))
        setVendors(simpleVendors)
      }
    } catch (error) {
  ...
  const handleModerationAction = async (action: 'approve' | 'reject' | 'correction') => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/venues/partners/${moderationPartner.id}${key}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleEdit = async (partner: VenuePartner) => {
    setFormData({
      name: partner.name,
      ...
      priceFrom: (partner as any).priceFrom?.toString() || '',
      ageFrom: (partner as any).ageFrom?.toString() || ''
    })
    
    // Загружаем всех вендоров для поиска
    try {
      const response = await fetch(`/api/admin/venues/vendors?key=${key}`)
      if (response.ok) {
        const data = await response.json()
        const vendorsData = data.vendors || data
        const simpleVendors = vendorsData.map((vendor: any) => ({
        ...
        const vendor = simpleVendors.find(v => v.id === partner.vendorId)
        setVendorSearch(vendor?.displayName || partner.vendor.displayName || '')
      }
    } catch (error) {
      console.error('Error loading vendors for edit:', error)
      setVendorSearch(partner.vendor.displayName || '')
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingPartner 
        ? `/api/admin/venues/partners/${editingPartner.id}?key=kidsreview2025`
        : `/api/admin/venues/partners?key=kidsreview2025`
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/admin/venues/partners/${id}${key}`, {
        method: 'DELETE',
      })
  ...
  const getVisibleParameters = () => {
    ...
    const visible = parameters.filter((param: any) => {
      if (!param.isActive) return false
      if (param.subcategoryId !== parseInt(formData.subcategoryId)) return false
  ...
  const getVisibleFilters = () => {
    ...
    const visible = filters.filter((filter: any) => {
      if (!filter.isVisible || !filter.isActive) return false
      return filter.subcategoryId === parseInt(formData.subcategoryId)
    })
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Кнопки навигации и действий - мобильная версия */}
      <div className="block md:hidden mb-6">
        <div className="flex flex-col gap-3">
          <Link 
            href={`/admin/venues${key ? `?key=${key}` : ''}`}
            className="w-full inline-flex items-center justify-center text-sm text-gray-600 hover:text-gray-900 transition-colors px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50"
          >
      ...
      {/* Кнопки навигации и действий - десктопная версия */}
      <div className="hidden md:flex items-center justify-end mb-6">
        <div className="flex items-center space-x-4">
          ...
          {/* Фильтр по городам - десктопная версия */}
      ...
      {/* Фильтр по городам - мобильная версия под заголовками */}
      <div className="block md:hidden">
        <div className="relative city-filter-dropdown">
          <button
            onClick={() => setShowCityFilter(!showCityFilter)}
            className="flex items-center space-x-2 bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors w-full justify-between"
          >
            <div className="flex items-center space-x-2">
              <span>🏙️ Города</span>
              {selectedCityIds.length > 0 && (
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Подкатегория * ({subcategories.length} загружено)
                  </label>
                  <select
                  ...
                  >
                    ...
                    {subcategories && subcategories.length > 0 ? subcategories.map((subcategory) => {
                      ...
                      <option value="" disabled>Загрузка подкатегорий... ({subcategories.length})</option>
                    )}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({ ...formData, tariff: e.target.value as 'FREE' | 'SUPER' | 'MAXIMUM' })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    required
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Оставьте пустым для бесплатного"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
