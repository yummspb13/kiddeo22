  ...
  const handleModerationAction = async (action: 'approve' | 'reject' | 'send_for_correction') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addToast({
          type: 'error',
          title: 'Ошибка при изменении статуса вендора',
      ...
      addToast({
        type: 'error',
        title: 'Ошибка при изменении статуса вендора',
        message: 'Произошла ошибка при изменении статуса вендора. Попробуйте еще раз.',
        duration: 6000
      })
    }
  }
  ...
  const handleCreateVendor = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addToast({
          type: 'error',
          title: 'Ошибка при создании вендора',
          message: errorData.error || 'Неизвестная ошибка',
          duration: 6000
        })
      }
      ...
      addToast({
        type: 'error',
        title: 'Ошибка при создании вендора',
        message: 'Произошла ошибка при создании вендора. Попробуйте еще раз.',
        duration: 6000
      })
    } finally {
      setLoading(false)
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        addToast({
          type: 'error',
          title: 'Ошибка при удалении вендора',
          message: 'Произошла ошибка при удалении вендора. Попробуйте еще раз.',
          duration: 6000
        })
      }
    } catch (error) {
                ...
                <button
                  onClick={() => {
                    ...
                    addToast({
                      ...
                      message: 'Функция отправки на доработку будет реализована в ближайшее время.',
                      duration: 5000
                    })
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                ...
                if (response.ok) {
                  ...
                  addToast({
                    type: 'error',
                    title: 'Ошибка при обновлении вендора',
                    message: 'Произошла ошибка при обновлении вендора. Попробуйте еще раз.',
                    duration: 6000
                  })
                }
              } catch (error) {
