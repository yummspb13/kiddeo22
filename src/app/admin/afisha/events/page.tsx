...
export default function AfishaEventsPage() {
  ...
  useEffect(() => {
    ...
    const loadEvents = async () => {
      try {
        ...
        setLoading(true)
        setError(null)
        const response = await fetch(`/api/admin/afisha/events?key=kidsreview2025&_t=${Date.now()}`, {
          cache: 'no-store',
          headers: {
  ...
  useEffect(() => {
    const loadQuickFilters = async () => {
      try {
        const response = await fetch(`/api/admin/quick-filters?key=kidsreview2025`)
        const data = await response.json()
  ...
  const handleEditEvent = (event: AfishaEvent) => {
    ...
    try {
      const quickFilterIds = (event as any).quickFilterIds ? JSON.parse((event as any).quickFilterIds) : []
    ...
    if (editingEvent) {
      try {
        ...
        const eventData = {
          ...
          richDescription: editingEvent.richDescription || null,
  ...
  const handleQuickPriorityChange = async (eventId: string, newPriority: number) => {
    try {
      ...
      const eventData = {
        ...
        city: event.city,
        category: (event as any).afishaCategory?.name,
      ...
      // Обновляем локальное состояние
      setEvents(events.map(event => 
        event.id === Number(eventId) ? { ...event, priority: newPriority } : event
      ))
    } catch (err) {
  ...
  const handleQuickPromotionToggle = async (eventId: string, isPromoted: boolean) => {
    try {
      ...
      // Обновляем локальное состояние
      setEvents(events.map(event => 
        event.id === Number(eventId) ? { ...event, isPromoted } : event
      ))
    } catch (err) {
  ...
  const handleDeleteEvent = async (id: string) => {
    if (confirm('Удалить событие?')) {
      try {
        ...
        const eventExists = events.find(event => event.id === Number(id))
        if (!eventExists) {
        ...
        const response = await fetch(`/api/admin/afisha/events/${id}?key=kidsreview2025`, {
          method: 'DELETE',
        })
        ...
        const refreshResponse = await fetch(`/api/admin/afisha/events?key=kidsreview2025&_t=${Date.now()}`, {
          cache: 'no-store',
          headers: {
            'Cache-Control': 'no-cache, no-store, must-revalidate',
            'Pragma': 'no-cache',
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Кнопки действий - десктопная версия */}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200" key={refreshKey}>
              {events.map((event) => (
                <tr key={String(event.id)} className="hover:bg-gray-50 cursor-pointer" onClick={() => handleEditEvent(event)}>
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-4">
                        ...
                        <div className="text-sm text-gray-500">
                          {(event as any).afishaCategory?.name} • {event.city}
                        </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="flex items-center gap-1 mr-2">
                        <button 
                          onClick={() => handleQuickPriorityChange(String(String(event.id)), 1)}
                        ...
                        <button 
                          onClick={() => handleQuickPriorityChange(String(event.id), 3)}
                          className={`px-2 py-1 text-xs rounded ${
                        ...
                        <button 
                          onClick={() => handleQuickPriorityChange(String(event.id), 5)}
                          className={`px-2 py-1 text-xs rounded ${
                      ...
                      {/* Кнопка продвижения */}
                      <button 
                        onClick={() => handleQuickPromotionToggle(String(event.id), !event.isPromoted)}
                        className={`px-2 py-1 text-xs rounded ${
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div>
                  ...
                  <RichTextEditor
                    content={editingEvent.richDescription || ''}
                    onChange={(content) => setEditingEvent({...editingEvent, richDescription: content})}
