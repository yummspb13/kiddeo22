// src/app/admin/afisha/filters/AfishaFiltersClient.tsx
"use client"
...
interface AfishaFiltersClientProps {
...
export default function AfishaFiltersClient({ 
  quickFilters: initialQuickFilters,
  adPlacements: initialAdPlacements,
  cities,
  keySuffix 
}: AfishaFiltersClientProps) {
  ...
  const handleQuickFilterSave = async (filterData: unknown) => {
    try {
      const url = editingQuickFilter 
        ? `/api/admin/quick-filters?key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}` 
        : `/api/admin/quick-filters?key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}`
      ...
      if (response.ok) {
        ...
        const [filtersResponse, placementsResponse] = await Promise.all([
          fetch(`/api/admin/quick-filters?key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}`).then(res => res.json()),
          fetch(`/api/admin/ad-placements?key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}`).then(res => res.json())
        ])
        
        const filters = Array.isArray(filtersResponse.quickFilters) ? filtersResponse.quickFilters : []
        const placements = Array.isArray(placementsResponse) ? placementsResponse : []
        
        setQuickFilters(Array.isArray(filters) ? filters : [])
        setAdPlacements(Array.isArray(placements) ? placements : [])
  ...
  const handleAdPlacementSave = async (placementData: unknown) => {
    try {
      const url = editingAdPlacement 
        ? `/api/admin/ad-placements?key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}` 
        : `/api/admin/ad-placements?key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}`
  ...
  const handleQuickFilterDelete = async (id: number) => {
    if (confirm('Удалить фильтр?')) {
      try {
        const response = await fetch(`/api/admin/quick-filters?id=${id}&key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}`, {
          method: 'DELETE'
        })
  ...
  const handleAdPlacementDelete = async (id: number) => {
    if (confirm('Удалить рекламный слот?')) {
      try {
        const response = await fetch(`/api/admin/ad-placements?id=${id}&key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}`, {
          method: 'DELETE'
        })
  ...
  const handleAdPlacementToggleActive = async (id: number, isActive: boolean) => {
    try {
      const response = await fetch(`/api/admin/ad-placements?key=${process.env.NEXT_PUBLIC_ADMIN_KEY || 'kidsreview2025'}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, isActive })
      })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Фильтры афиши</h1>
        <p className="text-gray-600 mt-1">Настройка быстрых фильтров и рекламных слотов</p>
      </div>
