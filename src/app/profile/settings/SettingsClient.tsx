...
export default function SettingsClient() {
  ...
  const currentSession = useMemo(() => 
    clientUser ? { user: clientUser } : session, 
    [clientUser, session]
  )
  ...
  // Принудительно обновляем данные пользователя при загрузке
  useEffect(() => {
    if (refetchUser) {
      refetchUser()
    }
  ...
  // Инициализируем поля из сессии только один раз
  useEffect(() => {
    if (currentSession?.user && !settings) {
      setFormData({
        name: currentSession.user.name || '',
        email: currentSession.user.email || ''
      })
      setSelectedAvatar(currentSession.user.image || '')
      setLoading(false) // Останавливаем загрузку, если есть данные в сессии
    }
  }, [currentSession?.user, settings])

  // Fallback: если API не работает, используем данные из сессии
  useEffect(() => {
    if (currentSession?.user && loading) {
      const timeout = setTimeout(() => {
        if (loading) {
          console.warn('API timeout, using session data as fallback')
          setFormData({
            name: currentSession.user.name || '',
            email: currentSession.user.email || ''
          })
          setSelectedAvatar(currentSession.user.image || '')
          setLoading(false)
          setError('Используются данные из сессии (API недоступен)')
        }
      }, 5000) // 5 секунд fallback

      return () => clearTimeout(timeout)
    }
  }, [currentSession?.user, loading])
  ...
  const handleAvatarSelect = async (avatarId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json()
        setSettings(data.user)
        setMessage('Аватар успешно сохранен')
        ...
        const errorData = await response.json()
        setError(errorData.error || 'Произошла ошибка при сохранении аватара')
      }
    } catch (error) {
      console.error('Error saving avatar:', error)
      setError('Произошла ошибка при сохранении аватара')
    }
  }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto mb-4"></div>
          <p className="text-gray-600 mb-2">Загрузка настроек...</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50 profile-content-mobile md:min-h-screen md:bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 profile-p-4-mobile md:max-w-4xl md:mx-auto md:px-4 md:sm:px-6 md:lg:px-8 md:py-8">
        <div className="bg-white rounded-lg shadow-sm p-6 profile-card-mobile md:bg-white md:rounded-lg md:shadow-sm md:p-6">
          <div className="flex justify-between items-center mb-6 profile-flex-mobile profile-items-center-mobile profile-justify-between-mobile md:flex md:justify-between md:items-center md:mb-6">
              <h1 className="text-2xl font-bold text-gray-900 profile-text-2xl-mobile md:text-2xl">Настройки профиля</h1>
          </div>
