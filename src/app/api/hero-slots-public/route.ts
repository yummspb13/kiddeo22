...
export async function GET(request: NextRequest) {
  console.log('Hero slots public API called')
  
  try {
    const { searchParams } = new URL(request.url)
    const city = searchParams.get('city')
    
    console.log('Hero slots public API called with city:', city)
    ...
    // Получаем активные слоты для города
    ...
    console.log('Returning events:', eventsWithPrice.length)
