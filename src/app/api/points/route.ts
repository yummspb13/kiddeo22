...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Быстрая загрузка только основных данных о баллах
    let recentTransactions = []
    let availableRewards = []
    let usedRewards = []
    
    // Загружаем транзакции с коротким таймаутом
    try {
      recentTransactions = await Promise.race([
        prisma.pointsTransaction.findMany({
          where: { userId },
          orderBy: { createdAt: 'desc' },
          take: 5 // Уменьшили с 10 до 5
        }),
        new Promise<any>((resolve) => setTimeout(() => resolve([]), 500)) // Уменьшили таймаут
      ])
    } catch (error) {
