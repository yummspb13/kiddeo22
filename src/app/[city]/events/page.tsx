...
import MobileEventsTimeline from '@/components/MobileEventsTimeline'
...
import MobileEventsDisplay from '@/components/MobileEventsDisplay'
...
async function getBaseFilters(props: {
  ...
  const where: any = {
    status: 'active',
    city: props.citySlug === 'moscow' ? 'Москва' : 
...
const getCachedPriceStats = unstable_cache(
  async (baseWhere: any) => {
    // Получаем все события для проверки цен
    ...
    return await Promise.all([
      { label: 'Бесплатно', range: { max: 0 } },
      { label: 'До 500 ₽', range: { min: 0, max: 500 } },
      { label: '500-1000 ₽', range: { min: 500, max: 1000 } },
      { label: '1000+ ₽', range: { min: 1000, max: null } }
    ].map(async ({ label, range }) => {
      // Фильтруем события по ценовым диапазонам
      const filteredEvents = events.filter(event => {
        // Считаем только от билетов, игнорируем isPaid
        const minPrice = getMinActivePrice(event.tickets, null)
        
        if (label === 'Бесплатно') {
          return minPrice === 0
        }
        
        // Бесплатные события не попадают в платные диапазоны
        if (minPrice === 0) return false
        if (minPrice == null) return false
        
        return inRange(minPrice, range)
      })
      
      return { label, count: filteredEvents.length }
    }))
  },
  ['price-stats'],
  { revalidate: 3600 } // 1 час кеш
...
async function loadAfishaEvents(props: {
  ...
  const events = await prisma.afishaEvent.findMany({
    ...
    select: {
      ...
      coverImage: true, gallery: true, tickets: true, city: true, afishaCategory: true, minPrice: true,
...
export default async function EventsPage({
  ...
  const { events, pagination } = await loadAfishaEvents({ citySlug: city, sp })

  // ОПТИМИЗАЦИЯ: Получаем базовые фильтры для кеширования
  const baseWhere = await getBaseFilters({ citySlug: city, sp })
  ...
  const [categoryStats, ageStats, priceStats] = await Promise.all([
    getCachedCategoryStats(cityData.name, baseWhere),
  ...
  const quickFilters = await prisma.quickFilter.findMany({
    where: {
      ...
      OR: [
        ...
        { cityId: cityData.id } // Фильтры для конкретного города
      ]
    },
    orderBy: { order: 'asc' }
  })
  ...
  return (
    <div className={`${unbounded.className} min-h-screen bg-white`}>
      {/* Mobile Categories Section - Full Width */}
      ...
      <main className="max-w-7xl mx-auto">
        ...
        {/* Collections Section */}
        <div className="px-2 sm:px-3 lg:px-4 mb-4">
          <CollectionsSection citySlug={city} />
        </div>

        {/* Рекламный слот 1 - полоска над "Все события" */}
        <div className="my-6 md:px-4">
          <HeaderBannerAd citySlug={city} />
        </div>

        {/* Timeline Filters Section */}
        <section className="mb-12 w-full">
          <div className="flex items-center mb-0 md:mb-6 py-4 md:px-4">
        ...
        <div>
          ...
          <div className="hidden lg:grid grid-cols-3 gap-8 px-4">
            ...
            <div className="col-span-1">
              ...
              <div className="bg-gray-200 rounded-lg h-64 mb-6 shadow-lg flex items-center justify-center">
                <AdSlot position="SIDEBAR_TOP" citySlug={city} className="w-[400px] h-[260px] rounded-2xl" />
              </div>
            ...
            <div className="col-span-2">
              {/* Results Header */}
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 px-4">
                <EventsCounter initialCount={pagination.totalEvents} citySlug={city} />
                
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  <EventsSort citySlug={city} />
          ...
          <div className="lg:hidden">
            {/* Results Header - Mobile Layout */}
            <div className="mb-6">
              {/* Строчка 1: Найдено N событий */}
              <div className="mb-4">
                <EventsCounter initialCount={pagination.totalEvents} citySlug={city} />
              </div>
              
              {/* Строчка 2: Сортировка и Вид */}
              <div className="flex items-center justify-between gap-4">
                {/* Столбец 1: Поле сортировки */}
                <div className="flex-1">
                  <EventsSort citySlug={city} />
                </div>
                
                {/* Столбец 2: Вид и кнопки */}
