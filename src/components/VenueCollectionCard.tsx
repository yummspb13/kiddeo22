...
interface VenueCollectionCardProps {
  venue: Venue
  index?: number
}

export default function VenueCollectionCard({ venue, index = 0 }: VenueCollectionCardProps) {
  const getPriceInfo = () => {
    return getPriceDisplay({
      tariff: venue.tariff as 'FREE' | 'SUPER' | 'MAXIMUM' | undefined,
  ...
  const getFirstImage = (coverImage?: string, additionalImages?: string) => {
    // Сначала пробуем coverImage
    if (coverImage) {
      return coverImage
    }
    
    // Если нет coverImage, пробуем additionalImages
    if (additionalImages) {
      try {
        const parsedImages = JSON.parse(additionalImages)
        if (Array.isArray(parsedImages) && parsedImages.length > 0) {
          const firstImage = parsedImages[0]
          // Если это объект с url, возвращаем url
  ...
  const firstImage = getFirstImage(venue.coverImage, venue.additionalImages)
        ...
        {(venue.averageRating && venue.averageRating > 0) && (
          <div className="absolute top-4 right-4 group-hover:scale-105 transition-transform duration-300">
            <div className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-white/90 backdrop-blur-sm text-gray-800 shadow-lg font-unbounded">
              ...
              <span className="font-bold">{venue.averageRating}</span>
              {venue.reviewsCount && venue.reviewsCount > 0 && (
          ...
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
              ...
              <span className="text-xs text-white/70 group-hover:text-white/90 transition-colors duration-300 font-unbounded">
                {getPriceInfo().subText || (venue.priceFrom || venue.priceTo ? 'за услугу' : 'вход свободный')}
              </span>
            </div>
