...
export default function VenueMap({ 
  ...
  useEffect(() => {
    ...
    const initMap = async () => {
      try {
        // Используем централизованную систему загрузки
        if (isYandexMapsReady()) {
          createMap()
        } else {
        ...
        setMapError('Ошибка загрузки карты. Проверьте API ключ.')
      }
    }
    ...
    const createMap = () => {
      try {
        ...
        (window as any).ymaps.ready(() => {
          ...
          const mapInstance = new (window as any).ymaps.Map(mapRef.current!, {
            center: [lat, lng],
            zoom: 15,
