...
export default function AdForm({ adPlacement, cities, keySuffix, isEdit = false, onSubmit }: AdFormProps) {
  ...
  const positions = [
    ...
    { value: 'SIDEBAR_TOP', label: 'Слева над фильтрами (400x260px)', description: 'Прямоугольный блок 400x260px слева над фильтрами' },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (onSubmit) {
        ...
        formData.append('adminKey', 'kidsreview2025')
        formData.set('imageUrl', imageUrl)
        formData.set('hrefUrl', hrefUrl)
        
        // Добавляем id для редактирования
        if (isEdit && adPlacement?.id) {
          formData.set('id', adPlacement.id.toString())
        }
        ...
        if (response.ok) {
          ...
          addToast({
            ...
            message: error.error || 'Неизвестная ошибка',
            duration: 6000
          })
        }
      }
      ...
      addToast({
        type: 'error',
        title: 'Ошибка сохранения рекламного слота',
        message: 'Произошла ошибка при сохранении рекламного слота. Попробуйте еще раз.',
        duration: 6000
      })
    }
  }
