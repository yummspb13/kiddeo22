...
export default function EventsList({ citySlug, initialEvents, searchParams }: EventsListProps) {
  ...
  const loadEvents = async (force = false) => {
    ...
    if (!citySlug) {
      console.error('EventsList: citySlug is not defined')
      return
    }
    ...
    try {
      ...
      console.log('EventsList: Fetching URL:', url)
      ...
      console.log('EventsList: Response status:', response.status)
      
      if (response.ok) {
        const data = await response.json()
        console.log('EventsList: Data received:', data)
        setEvents(data.events || [])
        setPagination(data.pagination || { page: 1, totalPages: 1, hasMore: false, totalEvents: 0 })
      } else {
        console.error('EventsList: Response not ok:', response.status, response.statusText)
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        console.log('EventsList: Request was aborted')
        return
      }
