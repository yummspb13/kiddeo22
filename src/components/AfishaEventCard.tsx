...
export default function AfishaEventCard({ event, citySlug = 'moskva' }: AfishaEventCardProps) {
  ...
  const formatTime = (dateStr: string) => {
    try {
      const date = new Date(dateStr)
      if (Number.isNaN(date.getTime())) {
        console.warn('Invalid date in formatTime:', dateStr)
    ...
    <Link href={`/event/${event.slug || event.id}`} className="group block" onClick={incrementViewCount}>
      ...
      >
        ...
        <div className="relative aspect-[16/9] overflow-hidden">
          ...
          {/* Бейдж "Закончилось" поверх фото (правый верхний угол) */}
          {isEventFinished() && (
            <div className="absolute right-3 top-3">
              <span className="inline-block px-2 py-1 bg-red-100/90 text-red-800 text-xs rounded-full backdrop-blur-sm font-medium">
                Закончилось
              </span>
            </div>
          )}
                ...
                price: event.tickets ? (() => {
                  try {
                    ...
                    if (Array.isArray(tickets) && tickets.length > 0) {
                      ...
                      if (prices.length > 0) {
                        const minPrice = Math.min(...prices)
                        return minPrice === 0 ? undefined : minPrice * 100 // Конвертируем в копейки
                      }
                    }
                  } catch {}
                  return undefined
        ...
        <div className="" style={{ padding: 'calc(var(--spacing) * 4)' }}>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <span className={`inline-flex items-center justify-center rounded-lg px-4 py-2 text-sm font-bold ${
              isEventFinished() 
